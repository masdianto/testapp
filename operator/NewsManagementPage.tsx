...
const NewsManagementPage: React.FC<NewsManagementPageProps> = ({ news, onSave, onDelete, isLoading }) => {
  ...
  const handleSaveAndClose = (newsItem: News) => {
    onSave(newsItem);
    if (!isLoading) {
      ...
      <div className="flex flex-col h-full">
        <header className="flex-shrink-0 flex items-center justify-between p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-3 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          >
